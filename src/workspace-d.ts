...
export class WorkspaceD extends EventEmitter {
    ...
    lint(document: AtomCore.IEditor): Thenable<LintResult[]> {
        ...
        return new Promise((resolve, reject) => {
            ...
            self.request({ cmd: "dscanner", subcmd: "lint", file: document.getPath(), ini: useProjectIni ? path.join(self.projectRoot, "dscanner.ini") : "" }).then(issues => {
                let diagnostics = [];
                if (issues && issues.length)
                    issues.forEach(element => {
                        let range = [[Math.max(0, element.line - 1), element.column - 1], [Math.max(0, element.line - 1), element.column + 300]];
                        console.log(range);
                        diagnostics.push({
    ...
    public setupDScanner() {
        this.request({ cmd: "load", components: ["dscanner"], dir: this.projectRoot, dscannerPath: atom.config.get("atomize-d.dscannerPath") || "dscanner" }).then((data) => {
    ...
    public startDCD() {
        this.request({
            ...
            this.request({ cmd: "dcd", subcmd: "setup-server", additionalImports: atom.config.get("atomize-d.stdlibPath") || ["/usr/include/dmd/druntime/import", "/usr/include/dmd/phobos"] }).then((data) => {
